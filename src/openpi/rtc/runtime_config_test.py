...
def test_checked_in_training_time_profile_disables_base_motion() -> None:
    profile_path = (
    ...
    assert config.robot.action_layout == "cartesian"
...
@pytest.mark.parametrize("timeout_s", [0.0, -1.0])
def test_parser_rejects_nonpositive_robot_idle_timeout(tmp_path: Path, timeout_s: float) -> None:
    runtime = copy.deepcopy(VALID_RUNTIME)
    runtime["control"]["robot_idle_timeout_s"] = timeout_s
    path = tmp_path / "bad.yaml"
    write_runtime(path, runtime)

    with pytest.raises(RuntimeConfigError, match="control.robot_idle_timeout_s must be positive"):
        load_runtime_config(path)
